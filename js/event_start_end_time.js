...
$("#event-start-time, #event-end-time").datetimepicker({ format: 'Y-m-d H:i', minDate: 0, minTime: 0, step: 30, onShow: AdjustMinTime, onSelectDate: AdjustMinTime });
...
$(".event-hours").on('blur', function(e) {
	if ($("#event-start-time").val() != "" && $("#event-end-time").val() != ""&& $("#hours").val() ==""){
var s1 = new Date($("#event-start-time").val());
var s2 = new Date($("#event-end-time").val());
var s3 = s2.getTime() - s1.getTime();
var tianshu = Math.ceil(s3/(24*60*60*1000)*8+s3%(24*60*60*1000)/(60*60*1000));
